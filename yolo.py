...
class YOLO(object):
    ...
    def detect_image(self, image):
        ...
        for i, c in enumerate(top_label):
            ...
            SaveImage = cv2.cvtColor(numpy.asarray(SaveImage), cv2.COLOR_RGB2BGR)
            SaveImageName = str(uuid.uuid1())
            SaveImageLabel = "kiven"
            Space = " "
            Xmin = str(left)
            Ymin = str(top)
            Xmax = str(right)
            Ymax = str(bottom)
            SaveTxtInfo = (
                        SaveImageName+Space+SaveImageLabel+Space+Xmin+Space+Ymin+Space+Xmax+Space+Ymax+'\n')
            cv2.imwrite("./SaveData/img/%s.jpg" % SaveImageName, SaveImage)  # 存储含有目标的图像（在画目标框之前存储，避免被画框）
            with open("./SaveData/txt/%s.txt" % SaveImageName, "a", encoding='utf-8') as txt:
                txt.write(SaveTxtInfo)

            # 画框框
            label = '{} {:.2f}'.format(predicted_class, score)
            # label = '{} {:.2f}'.format(predicted_class)
