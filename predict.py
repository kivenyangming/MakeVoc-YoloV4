...
if __name__ == "__main__":
    yolo = YOLO()
    mode = "video"  # predict表示单张图片预测;video'表示视频检测;fps'表示测试fps
    video_path = 0   # video_path用于指定视频的路径，当video_path=0时表示检测摄像头
    video_save_path = ""    # video_save_path表示视频保存的路径，当video_save_path=""时表示不保存
    video_fps = 25.0  # video_fps用于保存的视频的fps
    if mode == "predict":
        '''
        ...
        test_interval = 100
        img = PIL.Image.open('img/street.jpg')
